...
local DisableSeek = true
local NoDark = true
local Noclip = true
local DisableTimothy = true
local DisableA90 = true
local NoclipNext = true
local IsExiting = true
local RemoveDeathHint = true
local ClosetExitFix = true
local NoBreaker = true
local DisableEyes = true
local DisableGlitch = true
local DisableSnare = true
local WasteItems = true
local ScreechModule
...
local function ApplySettings(Object)
    task.spawn(function()
        ...
        if (ESP_Items[Object.Name] or ESP_Entities[Object.Name] or ESP_Other[Object.Name]) and Object.ClassName == "Model" then
            ...
            if Object.Name == "Vội Vã" or Object.Name == "Phục Kích" or Object.Name == "Mắt" or Object.Name == "A60" or Object.Name == "A120" then
                for i = 1, 100 do
        ...
        if Object:IsA("ProximityPrompt") then
            ...
            if IncreasedDistance and Object.Parent and Object.Parent.Name ~= "Kéo" then
        ...
        if Object.Name == "Mắt" then
            EyesOnMap = true
            if DisableEyes then
                MotorReplication:FireServer(0,-120,0,false)
            end
        end
        if Object.Name == "Gai" then
...
if Floor.Value == "Hotel" or Floor.Value == "Fools" then
    Tab:Toggle("Xoá bịp bợm","Makes it so you can't open duped doors",false,function(Bool)
        DisableDupe = Bool
    ...
    Tab:Toggle("Xoá Khung Cảnh Tìm Kiếm","Makes it so you can't trigger Seek to spawn. Other players still can.",false,function(Bool)
        DisableSeek = Bool
        for _,Object in pairs(workspace.CurrentRooms:GetDescendants()) do
    ...
    Tab:Toggle("Xoá Gai","Makes it so you won't get stunned or take damage from Snare when stepping on it.",false,function(Bool)
        DisableSnare = Bool
        for _,Object in pairs(workspace.CurrentRooms:GetDescendants()) do
...
Tab:Toggle("Không Mắt Máu Bởi Mắt","Makes the game (and other players) think you are looking down whenever eyes spawns.",false,function(Bool)
    DisableEyes = Bool
    if workspace:FindFirstChild("Mắt") then
...
Tab:Toggle("Không Mảnh Khối Điện","Tricks the game into thinking you completed the breaker puzzle at Room 100. May take up to 10 seconds to work.",false,function(Bool)
    NoBreaker = Bool
...
Tab:Toggle("Orther","Repeatedly uses everyone else's items like Vitamins, The Lighter, and The Flashlight.",false,function(Bool)
    WasteItems = Bool
    while task.wait(1) do
...
Tab2:Toggle("Esp Cửa Sách Khối Điện","Highlights all hostile entities.",false,function(Bool)
    OtherESP = Bool
    for _,Object in pairs(workspace:GetDescendants()) do
...
Tab2:Toggle("Xoá Giải Thích Cách Chết","Completely skips the Guiding/Curious light messages that appear after you die.",false,function(Bool)
    RemoveDeathHint = Bool
end)
Tab2:Toggle("Xoá Lỗi Thù","Removes the Glitch visual and sound. Will still teleport you.",false,function(Bool)
    DisableGlitch = Bool
end)
if Floor.Value == "Hotel" or Floor.Value == "Fools" then
    Tab2:Toggle("Nhện Không Thù","Removes the Timothy visual and sound. Will still deal damage.",false,function(Bool)
        DisableTimothy = Bool
    end)
end
